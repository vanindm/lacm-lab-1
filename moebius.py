...
root = Tk()
root.title("Поверхность Мёбиуса")
...
def changeAlpha(val):
    global polys
    polys = genPolys(moebius, 2 * pi, -0.5, 0.5, PRECISION, float(alpha.get()), float(beta.get()))
    draw()
...
alpha = Scale(orient=HORIZONTAL, length=200, from_=1.0, to=3.0, resolution=0.2, command=changeAlpha)
...
def moebius(u, v, alpha, beta):
    return np.array([(alpha + v * cos(u / 2) ) * cos(u), (alpha + v * cos(u / 2)) * sin(u), beta * v * sin(u / 2)])
